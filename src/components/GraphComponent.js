...
function nodesToCoords(data, lens) {
  let nodes = data.nodes
  let points = new Array(nodes.length)
  data.links.forEach(pair => {
    let source = nodes[+pair["source"]] === undefined ? pair["source"] : nodes[+pair["source"]]
    let target = nodes[+pair["target"]] === undefined ? pair["target"] : nodes[+pair["target"]]

    points.push(new Point2D(source["timestamp"], source[lens], target["timestamp"], target[lens]))
  })
...
export default function GraphComponent(props) {
  ...
  const renderScatterPlot = (data, canvas, lens) => {
    canvas.select('.y-axis').remove()
    canvas.select('.x-axis').remove()
    canvas.selectAll("circle").remove()
    canvas.selectAll("text").remove()
    let nodes = data.nodes
    let points = nodesToCoords(data, lens)
    nodes = nodes.filter(n => n.type === "comment")
    // find maximum property value
    let max = d3.max(nodes, n => {
      return +n[lens]
    })
    // find minimum property value
    let min = d3.min(nodes, n => {
      return +n[lens]
    })
    ...
    // Update selection
    const update = canvas.selectAll("circle")
      .data(nodes)

    update
      .attr("cx", d => {
        let cx = xscale(+d.timestamp)
        // console.log("cx: ", cx)
        return cx
      })
  ...
  useEffect(() => {
    ...
    if (props.layout === layout.forcelyaout || props.layout === layout.timeline) {
